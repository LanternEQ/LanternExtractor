...
namespace LanternExtractor.EQ
{
    ...
    {
        public static void Extract(string path, string rootFolder, Settings settings)
        {
            ...
            var archive = ArchiveFactory.GetArchive(path);
            string shortName = archiveName.Split('_')[0];

            if (!archive.Initialize())
            {
                Log.Error("LanternExtractor: Failed to initialize archive at path: " + path);
                return;
            }
            ...
            {
                Log.Error($"Unable to extract WLD file {wldFileName} from archive: {path}");
                return;
            }

            if (EqFileHelper.IsEquipmentArchive(archiveName))
            {
                ExtractArchiveEquipment(rootFolder, settings, wldFileInArchive, shortName, archive);
            }
            else if (EqFileHelper.IsSkyArchive(archiveName))
            {
                ExtractArchiveSky(rootFolder, settings, wldFileInArchive, shortName, archive);
            }
            else if (EqFileHelper.IsCharacterArchive(archiveName))
            {
                ExtractArchiveCharacters(path, rootFolder, settings, archiveName, wldFileInArchive, shortName,
                    archive);
            }
            else if (EqFileHelper.IsObjectsArchive(archiveName))
            {
                ExtractArchiveObjects(path, rootFolder, settings, wldFileInArchive, shortName, archive);
            }
            else
            {
                ExtractArchiveZone(path, rootFolder, settings, shortName, wldFileInArchive, archive);
            }

            MissingTextureFixer.Fix(archiveName);
        }

        private static void ExtractArchiveZone(string path, string rootFolder, Settings settings,
            string shortName, ArchiveFile wldFileInArchive, ArchiveBase archive)
        {
            ...
            var archiveLit = ArchiveFactory.GetArchive(path.Replace(shortName, shortName + "_lit"));
            WldFileZone wldFileLit = null;

            if (archiveLit.Initialize())
            {
                var litWldFileInArchive = archiveLit.GetFile(shortName + "_lit.wld");
                wldFileLit = new WldFileZone(litWldFileInArchive, shortName, WldType.Zone, settings);
                ...
                {
                    var lightsWldFile =
                        new WldFileLights(litWldLightsFileInArchive, shortName, WldType.Lights, settings,
                            wldFileLit);
                    lightsWldFile.Initialize(rootFolder);
                }
            }

            var wldFile = new WldFileZone(wldFileInArchive, shortName, WldType.Zone, settings, wldFileLit);
            ...
            {
                var lightsWldFile =
                    new WldFileLights(lightsFileInArchive, shortName, WldType.Lights, settings, wldFileLit);
                lightsWldFile.Initialize(rootFolder);
            }
            ...
            if (zoneObjectsFileInArchive != null)
            {
                WldFileZoneObjects zoneObjectsWldFile = new WldFileZoneObjects(zoneObjectsFileInArchive, shortName,
                    WldType.ZoneObjects, settings, wldFileLit);
        ...
        private static void ExtractArchiveObjects(string path, string rootFolder, Settings settings,
            ArchiveFile wldFileInArchive, string shortName, ArchiveBase archive)
        {
            ...
            var archiveObj2 = ArchiveFactory.GetArchive(path.Replace(shortName + "_obj", shortName + "_2_obj"));
            WldFileZone wldFileObj2 = null;

            if (archiveObj2.Initialize())
            {
                var obj2WldFileInArchive = archiveObj2.GetFile(shortName + "_2_obj.wld");
                wldFileObj2 = new WldFileZone(obj2WldFileInArchive, shortName, WldType.Zone, settings);
                wldFileObj2.Initialize(rootFolder, false);
            }

            var wldFile = new WldFileZone(wldFileInArchive, shortName, WldType.Objects, settings, wldFileObj2);
        ...
        private static void ExtractArchiveCharacters(string path, string rootFolder, Settings settings,
            string archiveName, ArchiveFile wldFileInArchive, string shortName, ArchiveBase archive)
        {
            ...
            {
                var archive2 = ArchiveFactory.GetArchive(path.Replace("global3_chr", "global_chr"));

                if (!archive2.Initialize())
                {
                    Log.Error("Failed to initialize archive at path: " + path);
                    return;
                }

                var wldFileInArchive2 = archive2.GetFile("global_chr.wld");

                wldFileToInject = new WldFileCharacters(wldFileInArchive2, "global_chr", WldType.Characters, settings);
                wldFileToInject.Initialize(rootFolder, false);
            }

            var wldFile = new WldFileCharacters(wldFileInArchive, shortName, WldType.Characters, settings, wldFileToInject);
        ...
        private static void ExtractArchiveSky(string rootFolder, Settings settings,
            ArchiveFile wldFileInArchive, string shortName, ArchiveBase archive)
        {
            var wldFile = new WldFileZone(wldFileInArchive, shortName, WldType.Sky, settings);
            InitializeWldAndWriteTextures(wldFile, rootFolder, rootFolder + shortName + "/Textures/",
                archive, settings);
        }

        private static void ExtractArchiveEquipment(string rootFolder, Settings settings,
            ArchiveFile wldFileInArchive, string shortName, ArchiveBase archive)
        {
            var wldFile = new WldFileEquipment(wldFileInArchive, shortName, WldType.Equipment, settings);
            ...
            InitializeWldAndWriteTextures(wldFile, rootFolder, exportPath, archive, settings);
        }

        private static void InitializeWldAndWriteTextures(WldFile wldFile, string rootFolder, string texturePath,
            ArchiveBase archive, Settings settings)
        {
            if (settings.ModelExportFormat != ModelExportFormat.GlTF)
            {
                ...
                WriteWldTextures(archive, wldFile, texturePath);
            }
        ...
        private static void WriteS3dSounds(ArchiveBase s3dArchive, string filePath)
        {
            ...
            foreach (var file in allFiles)
            {
                ...
                {
                    SoundWriter.WriteSoundAsWav(file.Bytes, filePath, file.Name);
                }
            }
        }
        ...
        /// <param name="filePath"></param>
        private static void WriteS3dTextures(ArchiveBase archive, string filePath)
        {
            ...
            foreach (var file in allFiles)
            {
                ...
                {
                    ImageWriter.WriteImageAsPng(file.Bytes, filePath, file.Name, false);
                }
            }
        }
        ...
        public static void WriteWldTextures(ArchiveBase archive, WldFile wldFile, string zoneName)
        {
            ...
            foreach (var bitmap in allBitmaps)
            {
                ...
                ImageWriter.WriteImageAsPng(pfsFile.Bytes, zoneName, bitmap, isMasked);
            }
        }
